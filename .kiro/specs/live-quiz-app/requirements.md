...
> **改訂履歴**: 当初は下位から1位まで自動タイマーのみで発表する設計だったが、ローカル確認の結果「参加者が多いと投影画面がスクロールされてしまう」「発表のペースを司会者が制御したい」という要望を受け、6位以下を5人単位のグループにまとめて主催者操作で発表を進める方式へ改訂した。上位5位以内の個別発表演出（下位ほど短い間、1位が最も長い、紙吹雪）は当初の設計を踏襲する。
...
3. When 主催者が次のグループを発表する操作を行った, the Host Console shall 現在表示中のグループを、次に下位から数えて新しいグループ（6位以下のグループが尽きていれば上位5位以内）へ切り替えて投影画面へ配信する。
4. While 上位5位以内を発表している, the Presentation Screen shall 1人ずつ、下位者よりも間を置いた演出で発表する（1位が最も間を置く）。
...
8. While 上位5位以内の発表段階に達していない, the Host Console shall 次のグループを発表する操作を提供する。上位5位以内の発表段階に達した後は、当該操作を提供しない（以降は要件15.4の演出のみで進む）。
