...
8. When 主催者が「プレビューを開く」を選択した, the Quiz Management Service shall 保存済みの外観設定と主催者自身が登録した設問(添付画像を含む)を用いたプレビューを別タブで開く。
9. While プレビューが開いている, the Quiz Management Service shall 画面送り操作により、投影画面(待機・出題・正解発表・中間ランキング・最終ランキング)と回答画面(ニックネーム入力・開始待ち・出題・結果)の代表的な状態を順に切り替えて表示する。
10. When 主催者がアップロード済みのロゴまたは背景画像の削除を指示した, the Quiz Management Service shall 当該画像への参照を解除し、以降は投影画面・回答画面・プレビューのいずれにも当該画像を表示しない状態で保存できるようにする。
11. While プレビューが投影画面の状態を表示している, the Quiz Management Service shall 実際のプロジェクター投影を想定した16:9のアスペクト比の枠内に表示する。
12. While プレビューが回答画面の状態を表示している, the Quiz Management Service shall スマートフォンを模した縦長のアスペクト比の枠内に、スクロール操作なしで全体を確認できる大きさで表示する。
13. The Presentation Screen shall プロジェクターのセーフティゾーンによる表示端の見切れを避けるため、画面端に既定の余白を設けて内容を表示する。
14. While プレビューが投影画面または回答画面の出題・正解発表の状態を表示している, the Quiz Management Service shall 主催者がイベントに登録した任意の設問を選択して表示内容を切り替えられるようにする。
...
> **改訂履歴1（Issue #29）**: 要件5.6は当初「正解と選択肢別の回答分布を表示する」とのみ定めており、表示に用いる値を規定していなかった。実装が選択肢ID（`crypto.randomUUID()`で採番されるUUID）をそのまま描画していたため、主催者が「どの選択肢が正解でどこに票が集まったか」を口頭で説明できず、実運用に支障があった。要件6.4で同じデータを扱う投影画面が選択肢の文言で表示しているのに対し、進行画面だけがこの変換を行っていなかったことによる。表示に用いる値・並び順・0票の扱い・正解の区別を要件として明記する。
>
> なお「出題中の進行画面にも選択肢一覧を表示するか」はIssue #29の補足で判断が保留されていたが、本改訂の対象外とした（バグ修正としての粒度を保つため。必要になった時点で別途起票する）。
...
6. When 回答受付が終了した, the Host Console shall 正解と選択肢別の回答分布を、選択肢の文言（ラベル）・設問で定義された並び順・人数と割合で表示し（回答が0件の選択肢も0人として表示する）、正解の選択肢を視覚的に区別できるようにしたうえで、正解発表操作を提供する。
...
> **改訂履歴1（Issue #28）**: 当初は要件7.6で正解発表のたびに途中順位（「現在の順位: N位」）を参加者へ表示していたが、要件15の最終結果発表の演出（下位から1位へ順に発表する）のネタバレになり、参加者体験としても不要であるため、途中順位の表示を取りやめた。要件7.9の最終順位表示は維持する。なお最小変更方針により、サーバーが配信する`PersonalResult.rank`（要件8.4で算出する順位）自体は残し、回答画面での表示のみを取りやめる。
...
6. When 回答受付が終了した, the Answer Screen shall 自分の回答の正誤と現時点の正解数を表示し、途中順位を表示しない（Issue #28による改訂）。
...
### Requirement 13: ニックネーム表示の安定性とランキング表示の見栄え（Issue #15）

**Objective:** As a 主催者/参加者, I want 長いニックネームや1位の星アイコン表示によって画面レイアウトが崩れないようにしたい, so that 会場での見栄えが損なわれず、安心して運用できる
...
**Objective:** As a 主催者/会場の参加者全員, I want 最終結果の発表に高揚感のある演出が欲しく、参加者数が多くても投影画面をスクロールせずに全員分の発表を進行できるようにしたい, so that 表彰式のような特別な瞬間として会場が盛り上がり、司会者が会場の空気を見ながら自分のペースで発表を進められる

> **改訂履歴1**: 当初は下位から1位まで自動タイマーのみで発表する設計だったが、ローカル確認の結果「参加者が多いと投影画面がスクロールされてしまう」「発表のペースを司会者が制御したい」という要望を受け、6位以下を5人単位のグループにまとめて主催者操作で発表を進める方式へ改訂した。
>
> **改訂履歴2**: 改訂履歴1の時点では、上位5位以内の個別発表（1人ずつ、下位者ほど短い間）は自動タイマーのままだった。ローカル確認の結果「6位以下と同様、上位5位以内も主催者の操作で1人ずつ発表を進めたい」という要望を受け、上位5位以内の個別発表も主催者操作（6位以下のグループ発表と同一の操作）で1人ずつ進める方式へ改訂した。これにより自動タイマーへの依存が完全になくなる。
>
> **改訂履歴3**: 改訂履歴1〜2までのグループ分割は、6位以下を「下位（最下位）から」5人単位に区切っていたため、端数（5で割り切れない余り）が6位側寄りの中間グループに生じ、1位から数えたときのグループ境界（1〜5位、6〜10位、…）と一致しなかった。ローカル確認の結果「1位から数えて5人単位（1〜5,6〜10,11〜15,…）の境界に揃えてほしい。端数は最下位側のグループになるようにしたい（例: 23人なら最初に発表される最下位グループは21〜23位）」という要望を受け、グループ分割の基準を「1位から数えて5人単位」に改訂した。発表順（最下位グループから上位グループへ、そして上位5位以内は1人ずつ）自体は改訂1〜2から変更しない。

#### Acceptance Criteria

1. When 最終ランキングが確定した, the Presentation Screen shall 表示対象の順位を、下位から上位へ向かって発表する。
2. While 発表対象に6位以下の順位が含まれる, the Presentation Screen shall 6位以下を「1位から数えて5人単位」（6〜10位、11〜15位、…）の境界でグループにまとめ、1グループのみを画面に表示し、他のグループを同時に表示しない（投影画面のスクロールを不要にするため）。参加人数が5の倍数でない場合の端数グループは、最下位側（最初に発表されるグループ）になる。
3. When 主催者が次を発表する操作を行った, the Host Console shall 現在の発表段階を1つ進め、投影画面へ配信する。6位以下のグループが残っている間は次のグループへ、上位5位以内に達した後は下位から1人ずつ、順位を1つ発表する。
4. While 上位5位以内を発表している, the Presentation Screen shall 主催者が次を発表する操作を行うたびに1人ずつ発表する（自動では進めない）。
5. When 1位が発表された, the Presentation Screen shall 祝福演出（既存の紙吹雪等）を表示する。
6. While 最終結果の発表演出の途中であり、ある順位がまだ発表されていない, the Presentation Screen shall 当該順位のニックネーム・正解数・合計回答時間を表示しない。
7. While 中間ランキングを表示している, the Presentation Screen shall 発表演出を行わず、要件6.5のとおり即時に表示する。
8. While 1位がまだ発表されていない, the Host Console shall 次を発表する操作を提供する。1位が発表された後は、当該操作を提供しない。
