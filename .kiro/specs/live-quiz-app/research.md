...
- **Verification**: 2026-09-14 に本番環境で実測し、この試算を裏付けた（100人・5問で DO 約127リクエスト＝0.13%、D1 約1,000行＝約1%）。ただし力点は修正が必要で、**比率で見ると D1 書込が DO リクエストより約8倍タイト**であり、実質的な制約はメッセージ頻度ではなく D1 の日次書込上限にある。詳細は `docs/load-test/2026-09-14-production.md` を参照。
...
- **Selected Approach**: D1 が「カタログ（アカウント・イベント・設問・外観設定）」と「確定済み結果アーカイブ」を所有。DO が「開催中のライブ状態（参加者・回答・進行フェーズ）」を所有。取り込みは**二段階**とし、`publish` 時に定員・状態・外観を、`startSession` 時に設問を凍結して取り込む。確定時に結果を D1 へ書き戻す。
- **Rationale**: 各データに単一の所有者が定まり、要件1.6（開催中の設問変更禁止）がスナップショット凍結によって自然に強制される。参加受付は `startSession` より前に始まる（要件5.1）ため、定員と状態だけは先に DO へ渡す必要がある。
...
- **Follow-up**: `event.status` は D1 に置かれるが遷移の駆動元は DO 側の出来事であるため、`startSession` / `finalize` 時に DO から書き戻す。書き戻しが失われると要件1.6・3.7 が例外を出さずに失効するため、回帰テストで保護する。
